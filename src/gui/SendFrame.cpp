...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("arto://", Qt::CaseInsensitive))
    {
       _request.replace(0, 7, "arto:");
    }
    if(!_request.startsWith("arto:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with arto:"), QtCriticalMsg));
      return;
    }
